...
エンジニアリングの知識がなくても、以下の手順で簡単に起動できます。

1.  プロジェクトフォルダ内にある **「管理画面を起動する.bat」** をダブルクリックします。
2.  黒いウィンドウ（コマンドプロンプト）が立ち上がります。※文字化けを避けるため英語でメッセージが表示されますが、そのままお待ちください。
3.  **3秒後に自動的にブラウザで管理画面が開きます。**
4.  作業が終わるまで、その黒いウィンドウは閉じずにそのままにしておいてください（最小化してOKです）。

> [!TIP]
> **デスクトップにショートカットを作る**:
> 「管理画面を起動する.bat」を右クリック ＞ 「送る」 ＞ 「デスクトップ（ショートカットを作成）」を選択すると、次からデスクトップのアイコンをカチカチするだけで起動できるようになります。
